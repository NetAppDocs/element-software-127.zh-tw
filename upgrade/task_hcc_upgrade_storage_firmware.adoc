...
NOTE: 如需硬體韌體與驅動程式韌體的完整對照表link:../hardware/fw_storage_nodes.html["支援的儲存韌體版本SolidFire 、適用於不支援的儲存節點"]、請參閱。
...
.步驟
. 將儲存韌體升級套件下載至管理節點可存取的裝置；前往 Element 軟體 https://mysupport.netapp.com/site/products/all/details/element-software/downloads-tab["下載頁面"^]並下載最新的儲存韌體映像。
. 將儲存韌體升級套件上傳至管理節點：
+
