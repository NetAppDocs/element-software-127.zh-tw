...
| `storageNodeIds` | UUID陣列 | 如果 `storageLogs`設為 `true`、請使用儲存叢集節點 ID 填入此參數、將記錄收集限制在特定儲存節點。使用 `GET https://[management node IP]/logs/1/bundle/options`端點來查看您可以使用的所有可能節點 ID 。 
|===
