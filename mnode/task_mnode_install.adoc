...
.. 為每個要新增的預設時間伺服器新增一行。預設時間伺服器必須與您將在 <<set-up-the-management-node,後續步驟>> 中使用的儲存設備叢集上的 NTP 伺服器相同。
+
[listing]
----
vi /etc/ntp.conf
...
*** *--storage_mvip (-sm) [MVIP 位址]*: 執行 Element 軟體的儲存設備叢集的管理虛擬 IP 位址 （MVIP）。請使用與您在 link:task_mnode_recover.html#configure-time-sync["NTP伺服器組態"] 期間相同的儲存設備叢集來設定管理節點。
